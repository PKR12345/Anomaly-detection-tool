...
#         st.write('w1: ',tf.reduce_min(w1).numpy(), tf.reduce_max(w1).numpy())
#         st.write('w2: ',tf.reduce_min(w2).numpy(), tf.reduce_max(w2).numpy())
#         st.write('w3: ',tf.reduce_min(w3).numpy(), tf.reduce_max(w3).numpy())
        loss1 = 1/pow(1.1,n) * tf.reduce_mean(tf.square(tf.math.multiply(batch - w1,sample_weights))) + (1/1.1 - 1/pow(1.1,n)) * tf.reduce_mean(tf.square(tf.math.multiply(batch - w3,sample_weights)))
        loss2 = 1/pow(1.1,n) * tf.reduce_mean(tf.square(tf.math.multiply(batch - w2,sample_weights))) - (1/1.1 - 1/pow(1.1,n)) * tf.reduce_mean(tf.square(tf.math.multiply(batch - w3,sample_weights)))
...
def training(epochs, model, train_dataset, val_dataset, sample_weights_train, sample_weights_val, batch_size, lr, seq_len, opt_func=tf.optimizers.Adam):
    history = []
    optimizer1 = opt_func(learning_rate=lr, clipvalue=1.0)
    optimizer2 = opt_func(learning_rate=lr, clipvalue=1.0)
...
#     @tf.function
    def step_forward(batch,sample_weight,epoch):
...
#                 st.write("epoch",epoch,"batch no",i,loss1,loss2)
    ...
    for epoch in range(epochs):
        for i in range(len(train_dataset)):
            ...
            step_forward(batch,sample_weight,epoch)
...
# @tf.function
# def step_forward(model, batch, sample_weight, epoch, optimizer1, optimizer2):
#     with tf.GradientTape(persistent=True) as tape:
#         # Train AE1
#         loss1, loss2 = model.training_step(batch, sample_weight, epoch + 1)
...
# def training(epochs, model, train_dataset, val_dataset, sample_weights_train, sample_weights_val, batch_size, lr, seq_len, opt_func=tf.optimizers.Adam):
#     history = []
#     optimizer1 = opt_func(learning_rate=lr, clipnorm=1.0)
#     optimizer2 = opt_func(learning_rate=lr, clipnorm=1.0)
#     train_dataset, val_dataset, sample_weights_train, sample_weights_val= batch_generator(train_dataset,batch_size), batch_generator(val_dataset,batch_size), batch_generator_mask(sample_weights_train,batch_size), batch_generator_mask(sample_weights_val,batch_size)
...
#             step_forward(model, batch, sample_weight, epoch, optimizer1, optimizer2)
...
#                         validation_data=(X_val, X_val,X_mask_model_tv), shuffle=False, callbacks=[tf.keras.callbacks.EarlyStopping(monitor='loss', mode='min',patience=12)])
    ...
    history, val_loss= training(15, model, X_train, X_val, X_mask_model_tm, X_mask_model_tv, int(params["batch_size"]), float(params["lr"]), tf.optimizers.Adam)
